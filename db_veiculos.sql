...
CREATE TABLE IF NOT EXISTS `veiculosbd`.`tb_aluguel` (
  ...
  `kminicial_aluguel` DECIMAL(10,1) NOT NULL,
  ...
  INDEX `fk_tb_aluguel_tb_pagamento1_idx` (`tb_pagamento_id_pagamento` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_tb_aluguel_tb_pagamento1`
    ...
    REFERENCES `veiculosbd`.`tb_pagamento` (`id_pagamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
